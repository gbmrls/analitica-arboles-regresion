...
```{r}
final_board_games <- final_board_games %>% filter(max_players %in% c('0','9','12','13','15','18','24','99'))

final_board_games %>%  ggplot() +
  geom_bar(mapping = aes(x = max_players)) +
  ggtitle("Count of Board Games by max_players")
```
...
final_board_games_n <- final_board_games[c(4,5,6,7,8,10,19,20)]
final_board_games_rpart <- as.data.frame(final_board_games_n)
split = sample.split(final_board_games_rpart,SplitRatio=0.8) # Los datos de train serán de 80%
train_rpart = subset(final_board_games_rpart,split == TRUE)
test_rpart = subset(final_board_games_rpart,split == FALSE)
```
